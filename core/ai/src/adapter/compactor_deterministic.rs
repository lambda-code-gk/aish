...
pub(crate) fn maybe_compact(fs: &dyn FileSystem, session_dir: &Path) -> Result<(), Error> {
    ...
    let records = session_manifest::load_all(fs, session_dir)?;
