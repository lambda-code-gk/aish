...
継続メッセージは Human Task result が unverified であること、環境を再観測すること、completion criteria を検証してから完了を主張することを固定指示する。成功時は `Continuing→Finished` を保存してから checkpoint を削除する。provider/socket/turn error は同じ ID を保持した `ResultPending` へ戻し、Finished save 前は削除しない。delete 失敗は有効な Finished checkpoint を残し、status と cancel cleanup を可能にする。process crash で Continuing が残る場合の自動回復は 0063-E であり、本 spec は行わない。

wire `RequestContext.continuation_turn=true` は aibe の既存 request service に process-local admission を指示する。aibe は active ID と、正常完了した continuation ID の `HashSet` を process memory に保持し、同じ ID の並行開始または成功後の再受理を LLM 実行前に拒否する。provider error 等の失敗 ID は completed 集合へ入れず、同じ ID の明示 retry を許す。これは永続正本、journal、lease、reconciler、aibe 再起動跨ぎの exactly-once ではない。
