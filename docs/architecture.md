...
応答は `summary` / `proposals[]`（`operation` + 表示専用 `rationale`）/ `applied_entries`。LLM profile は AgentTurn と同じ default。`apply=true` は validation 済み operation のみ store へ反映。CLI `--apply` は提案表示後に対話確認（非対話 stdin は fail-closed）。shell execute とは無関係。
