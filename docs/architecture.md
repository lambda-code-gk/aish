...
- **Basic プロファイル（0038 Phase A）**: `[memory] enabled = false`（aibe / ai 各 config）で contextual memory を無効化。`agent_turn` 注入なし、memory RPC 拒否、`ai goal` 等 CLI 拒否。環境変数 `AIBE_MEMORY_ENABLED` / `AI_MEMORY_ENABLED` で上書き可。詳細: [spec/0038](spec/0038_contextual-memory-pack-phase-a-spec.md)。
