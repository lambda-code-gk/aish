...
Worker Result、exit 0、changed paths、Artifact locator は `source=agent_task, verified=false` のままである。親が同じ tool 経路で再観測し、委譲前 plan の command/args と完全一致して **実プロセス実行された** `shell_exec` を対象 criterion 限定の `Verification` とする（`verified=true` は成功時のみ。非 zero は `verified=false` の観測で Gap 経路へ）。通常 shell は引き続き `UnknownShellEffect` で、過去の observation/verification を stale 化する。検証 command を先、成果物 `read_file` / Git 観測を後に実行する。MVP では command plan item は1件まで。Worker・Gap・output から plan や command を追加しない。
