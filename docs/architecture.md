...
- **提案コマンド再呼び出し**（`0053`）:
  ...
  - `aish shell` の replay `DEBUG` trap は `Alt+.` / `Alt+,`（`bind -x`）実行中に start span を出さない。`FUNCNAME` で `_ai_recall_*` / `_aish_handoff_recall_apply` を除外する（DEBUG は simple command 前に走るため flag だけでは不足）
  - `_AISH_HUMAN_SHELL=1` かつ handoff 候補がある場合だけ、0055 の一時 rcfile が後段で同じ shortcut を handoff 固定候補へ bind する。候補なしまたは Human Shell 外では 0053 / ユーザーの既存 binding を上書きしない
