...
- **Compile-time packaging（0038 Phase D）**: `aibe` / `ai` は `memory` Cargo feature（default 有効）を持つ。`--no-default-features` では contextual memory 実装をリンクせず basic build を成立させる。memory 固有実装は各 crate 内の `plugin_memory/` モジュール（`#[cfg(feature = "memory")]`）に集約し、`application/` は facade + `BasicPack` / fail-closed stub を残す。`aibe/src/plugin_memory/` に `ContextualMemoryPack` と memory service 群、`ai/src/plugin_memory/` に `MemoryCliPack` / policy / handler を置く。runtime の `memory.enabled`（Phase A）と compile-time の `memory` feature は別軸（混同しない）。詳細: [spec/0038 Phase D](spec/0038_contextual-memory-pack-phase-d-spec.md)。
