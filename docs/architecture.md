...
| **ai** | `aibe-client` + `aibe-protocol` + `aish-replay` 経由で aibe に接続し応答を表示。`shell_exec` 承認 UX（`y/n/a/c`、tier 分類、session cache、`--yes-exec`、`[tools.shell_exec.auto_approve_patterns]`）と `aish.replay_show` の client-side 実行は **ai** の責務。`--collaborative` では `CollaborativeShellExecPolicy` が承認 UI の前で親 `shell_exec` を human shell handoff に変換する。wire 上は `ShellExecApproval.approval_origin` で provenance を aibe に渡す（0036/0055）。transport は `aibe-client` | aibe デーモンのみ（LLM 直叩き禁止） |
...
0055 の handoff durable state は `ai` の `CollaborativeWorkflow` aggregate を正本とする。`workflow.json` に handoff、checkpoint、pending effect、revision をまとめ、filesystem adapter が revision CAS と atomic replace を行う。application は `save_handoff` / `save_checkpoint` を呼ばず workflow mutation だけを使う。外部副作用は effect を先に永続化し、`CollaborativeWorkflowReconciler` が stale claim 回収と冪等再試行を担当する。token plaintext は shell launch の process-local input に限定し、aggregate / effect / audit へ保存しない。
...
- Collaborative handoff（0055 Phase 2）では `RequestContext.collaborative_handoff=true` により server の通常 approval mode に依存せず client execution-policy gate を選び、`ShellExecApprovalOrigin::CollaborativeHandoff` と optional `handoff_result: HumanHandoffResult` を返す。`handoff_result` がある場合、aibe は subprocess を起動せず、`execution_outcome=human_control_returned` / `requested_command_completion=unknown` の synthetic tool result を次の LLM round へ渡す。`ai` は checkpoint 保存後に `aish human-shell` を同期起動し、`aish` は PTY control channel の正常返却マーカー（Ctrl+D / `exit` / `exit N`）と final cwd を返す。通常モードと PARENT 以外は既存実行経路を使う。
- Phase 4 の復旧では `aish human-shell` supervisor が prompt 描画と独立した heartbeat で既存 lease を原子的に延長する。lease expiry 単独では状態遷移せず、owner process 消失または shell の異常終了を確認した場合のみ `ORPHANED` にする。`ai resume` は `ORPHANED` では token rotation と shell generation 更新後に新しい human shell を起動し、`RETURNED` では shell を起動せず checkpoint の保留 `shell_exec` と会話要約から新しい parent run を意味的に開始する。中断時に `RUNNING` だった tool は `UNKNOWN` として保存し、自動再実行しない。
