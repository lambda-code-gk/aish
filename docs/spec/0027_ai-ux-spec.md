...
- `--format` は **stdout の最終表現だけ**を決める。assistant streaming の delta を stdout に流すかどうかは `ai` の表示層責務であり、structured output 時は delta を抑止して最終 structured 出力のみを出す（0033）。
...
- progress は stderr の責務であり、`--format` 指定の有無とは独立に扱う（0033）。turn スコープの spinner は RAII（`ProgressGuard`）で必ず停止する。

### C-3. assistant streaming

- assistant token / chunk を逐次表示する。
- human 向け（`--format` 未指定）のときは stdout に逐次表示する。`--format json|tsv|env` 時は delta を stdout に出さず、最終 structured 出力だけを出す（0033）。
