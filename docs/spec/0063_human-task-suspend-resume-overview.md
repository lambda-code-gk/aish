...
| 0063-C | `ai human-task resume`、新segment、複数回中断、DoneのResultPending永続化 | 正式 spec [`0064_human-task-resume-spec.md`](0064_human-task-resume-spec.md)。agent continuation は含めない。resume cwd 不在時は Suspended を維持 |
| 0063-D | 保存した親コンテキストと最終結果から新しい Collaborative Mode 継続ターンを開始 | 正式 spec [`0065_human-task-agent-continuation-spec.md`](0065_human-task-agent-continuation-spec.md)。`ResultPending` と同一process内重複防止を扱う |
| 0063-E | stale ownership、予期しないprocess終了、破損checkpointの追加回復UX、権限・session pruning競合の hardening | 正式 spec [`0066_human-task-recovery-hardening-spec.md`](0066_human-task-recovery-hardening-spec.md)。手動 recovery に限定し、crash recovery / lease / migration が必要なら STOP-THE-LINE |
