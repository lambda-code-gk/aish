...
- Windows 対応
- TOML `[features]` セクションの parse / 読み込み実装
...
この差は kind / recipe / feature のいずれにも適用する。ただし 0043 Phase 3 では、kind / recipe / feature を 1 つの暗黙バンドルとして扱わず、pack ごとの設定面を分離したうえで、`None` の互換解釈だけを明示的な resolver に閉じ込める。
...
将来、memory / feature の設定は `[features]` セクションへ分離する。ただし本 Phase では `[features]` TOML 節の parse / 読み込みは行わず、予約済みの設定面としてのみ扱う。

#### 5.3.1 目的

- feature pack を memory pack から独立した設定面として扱う
- `kind_files` / `recipe_files` / `feature_files` の関係を、暗黙の bundle ではなく明示的なモデルとして表現する
- Phase 2 の挙動を維持したまま、将来の `[features]` 分離を機械的に移行できるようにする

#### 5.3.2 非目標

- TOML `[features]` セクションの parse / 読み込み
- `RoutePlan` wire や `FeatureAction` schema の破壊的変更
- `shell_exec` を `recommended_tools` に戻すこと
- feature pack を memory pack より弱い設定として再従属させること

#### 5.3.3 設定モデル

Phase 3 では設定を次の 2 面に分けて扱う。

- `MemoryPackConfig`: `enabled`、`kind_files`、`recipe_files`
- `FeaturePackConfig`: `feature_files`

`FeaturePackConfig` は feature registry の入力を 1 か所に集約し、`kind_files` / `recipe_files` を直接参照しない。`kind_files` / `recipe_files` は memory pack の責務に留め、feature pack 側は「有効か」「どの feature 定義を列挙するか」だけを持つ。

legacy 互換のため、現行の `[memory].feature_files` は `FeaturePackConfig` への入力として受けるが、意味づけは feature pack 側へ移す。`feature_files=None` は互換モード、`feature_files=[]` は明示的な空 registry とする。

#### 5.3.4 コード構造

- composition root は `MemoryPackConfig` と `FeaturePackConfig` を別々に組み立てる
- composition root は `memory.enabled`、`kind_files`、`recipe_files`、`feature_files` から `effective_feature_mode` を 1 回だけ解決する
- `FeatureRegistryLoader` は解決済みの `effective_feature_mode` と `FeaturePackConfig` だけを受け取り、`kind_files` / `recipe_files` / `memory.enabled` を直接見ない
- `memory.enabled=false` のときは composition root が `FeatureRegistry::empty()` を渡す
- `feature_files=None` の互換解釈は composition root が持ち、generic memory のときは empty registry、legacy 互換のときだけ baseline pack を使う
- `FeaturePackConfig` 自体は `feature_files` のみを保持し、`memory.enabled` は保持しない

#### 5.3.5 Phase 2 挙動の維持方針

- `memory.enabled=false` は引き続き basic aibe として empty registry を返す
- `kind_files=[]` / `recipe_files=[]` / `feature_files=[]` は generic memory の明示形として維持する
- `feature_files=None` は full AISH pack の互換モードを維持する
- `feature_files=None` でも generic memory を選んだ場合は baseline feature を戻さない
- `priority` / `requires_memory` / `requires_recipe` による eligibility ルールは Phase 2 のまま維持する

#### 5.3.6 受け入れ条件

- feature pack の有効 / 無効は memory pack から独立して説明できる
- `kind_files` / `recipe_files` の変更だけで feature registry の意味が暗黙に変わらない
- `feature_files=[]` は常に空 registry を意味し、baseline pack に落ちない
- `feature_files=None` の互換モードは、Phase 2 の generic memory 保護を壊さない
- `[features]` TOML 節が未導入でも、設計上の責務分離が成立する

#### 5.3.7 テスト方針

- unit: `FeaturePackConfig` の解決結果が `feature_files=None` / `[]` / 非空で正しく分岐すること
- unit: `effective_feature_mode` の解決が `memory.enabled` と pack 状態で一意になること
- unit: `FeatureRegistryLoader` が `MemoryPackConfig` に依存せず、解決済みの feature mode と feature 定義のみを見ること
- integration: `memory.enabled=false` では feature registry が empty のままであること
- integration: generic memory では kind / recipe は空でも feature pack の明示空が維持されること
- integration: full AISH pack では baseline feature が従来どおり読まれること
- regression: Phase 2 の eligibility と read-only tools の挙動が変わらないこと
- docs: `docs/architecture.md` と `docs/testing.md` を同一変更で同期すること

## 6. `route_turn` と `ai` の契約

### 6.1 `aibe/src/application/server.rs`

composition root は `memory.enabled` と pack 状態から `effective_feature_mode` を解決し、feature registry の初期値を決める。

- `memory.enabled=false` なら `FeatureRegistry::empty()` を渡す
- `memory.enabled=true` なら解決済み mode に従って loader を使って registry を構築する
...
| Phase 3 unit | `FeaturePackConfig` が memory pack から独立して解決されること |
| Phase 3 integration | `feature_files=None` と generic memory の組み合わせで baseline feature が復活しないこと |
| Phase 3 regression | Phase 2 の eligibility / read-only / empty registry の受け入れ条件が維持されること |
...
- feature pack は memory pack から独立した設定面として記述できる
- `feature_files=None` の互換モードは維持されるが、`[features]` TOML 節の parse / 読み込みは行わない
- generic memory の設定は Phase 2 と同じく baseline feature を復活させない
- Phase 2 の eligibility / read-only tools / empty registry の受け入れ条件がすべて維持される
- `./scripts/verify.sh` が通る
