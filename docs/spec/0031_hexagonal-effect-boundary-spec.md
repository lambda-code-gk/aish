...
### composition root 例外との組み合わせ

現在 `server.rs` は composition root 例外のため、

```bash
check_crate aibe server.rs
```

によって `application -> adapters` の `use` も許されている。

つまり現状のチェックは次の状態。

```text
server.rs は例外だから adapters use は OK
std::fs / tokio::net / libc はそもそも見ていない
```
...
`severity = "warn"` のルールも恒久化を防ぐため、設計書「Phase ごとの severity 正本」に **昇格先 phase** を明記する（例: `ports.no-env` → Phase 3 以降で `config.rs` 修正後に `fail` 昇格）。昇格前に `warn` を無期限放置しない。
...
### domain 層（Phase 1: すべて `fail`、既知違反は allowlist）
...
Phase 3 で `domain.no-env` / `domain.no-filesystem` の severity を `warn` に変更し、allowlist を削除して可視化する（下記「Phase ごとの severity 正本」）。

### ports 層（段階的導入）

`ports` は trait 定義中心にしたいが、現状 `aibe/src/ports/outbound/config.rs` に `std::env::var("HOME")` がある。設計上は adapter 側へ出した方がよいが、いきなり `fail` にすると修正範囲が増える。

| ID | Phase 1 severity | 禁止パターン |
|----|------------------|-------------|
...
| `domain.no-env` | `fail` + allowlist | `warn`、allowlist 削除 | Phase 3 で CI に警告を出す |
| `domain.no-filesystem` | `fail` + allowlist | `warn`、allowlist 削除 | 同上 |
| `domain.no-process` 他 | `fail` | `fail` | 既知違反なし |
| `ports.no-process` / `ports.no-http` | `fail` | `fail` | 変更なし |
| `ports.no-filesystem` / `ports.no-env` | `warn` | `warn` → 修正後 `fail` 昇格 | `config.rs` 修正タスクと連動 |

Phase 1 の `hexagonal-rules.toml` は **application / domain / ports すべてのルールを最初から入れる**。domain の既知違反は allowlist で抑え、Phase 3 で allowlist を外して `warn` に切り替える。
...
### Phase 3 正本（domain severity 変更分のみ）

Phase 3 では `hexagonal-rules.toml` の次 2 ルールの `severity` を `warn` に変更し、対応する allowlist エントリを削除する。

```toml
# Phase 3: domain.no-env / domain.no-filesystem の severity を fail → warn に変更
# allowlist から ai/src/domain/llm_profile.rs と shell_log_resolve.rs を削除
```

### 本ルールで検出される既知違反

| ファイル | 検出ルール | Phase 1 の扱い |
|---------|-----------|----------------|
| `aibe/src/application/server.rs` | `application.no-unix-socket`, `application.no-filesystem-io`, `application.no-libc` | **allowlist 禁止**（Phase 2 で修正） |
| `ai/src/application/ask.rs` | `application.no-env` | allowlist（Phase 3 で修正 or 削除） |
| `ai/src/domain/llm_profile.rs` | `domain.no-env` | allowlist（Phase 3 で warn 可視化） |
| `ai/src/domain/shell_log_resolve.rs` | `domain.no-filesystem` | allowlist（Phase 3 で warn 可視化） |
| `aibe/src/ports/outbound/config.rs` | `ports.no-env` | `warn` でそのまま出力（allowlist 不要） |

## allowlist 例（正本）

```toml
# scripts/hexagonal-allowlist.toml
# line は checker が報告する行番号と一致させる（1 違反行 = 1 エントリ）

[[allow]]
rule = "application.no-env"
path = "ai/src/application/ask.rs"
line = 77
reason = "existing violation; move client_cwd resolution to main.rs / composition root"
remove_by = "phase:3"
...
line = 10
reason = "existing violation; move env lookup to CLI/composition root"
remove_by = "phase:3"
...
line = 3
reason = "existing violation; use std::fs::File in domain"
remove_by = "phase:3"
...
path = "ai/src/domain/shell_log_resolve.rs"
line = 110
reason = "existing violation; std::fs::metadata in domain"
remove_by = "phase:3"
```

**allowlist に入れないファイル**: `aibe/src/application/server.rs`（本命違反。Phase 2 で構造修正する）。

**allowlist から外すタイミング**: Phase 3 で domain / `ask.rs` の修正と同時、または `warn` 可視化の直前に削除する。
...
### Phase 3: 既存 domain / application 違反の整理
...
1. `hexagonal-rules.toml` で `domain.no-env` / `domain.no-filesystem` の severity を `fail` → `warn` に変更
2. `hexagonal-allowlist.toml` から domain 2 件と `ask.rs` の `application.no-env` エントリを削除
3. 違反を修正するか、`warn` 出力を確認してから `fail` 昇格を検討

**受け入れ条件**:

- domain 既知違反が `HEXAGONAL WARN` として CI ログに出ること（CI は pass のまま）
- `ask.rs` の `application.no-env` は修正後 allowlist 不要、未修正なら `fail` で落ちること
- 修正完了後は該当ルールを `fail` に昇格できること
...
3. `ai/src/application/ask.rs` の `application.no-env` 違反が検出される（Phase 1: allowlist、Phase 3: 修正 or fail）
4. `aibe/src/ports/outbound/config.rs` の `ports.no-env` が `warn` で検出される（Phase 1 から）
...
- `ports.no-filesystem` / `ports.no-env` を `fail` に昇格するタイミング（`aibe/src/ports/outbound/config.rs` の `HOME` 参照修正タスクと連動）
- Phase 2 の composition root の最終配置（`main.rs` vs 専用 `composition.rs`）は実装時にファイルサイズとテスト容易性で決定
- PTY 関連（`aish` の `pty_shell.rs` 等）を effect boundary ルールに含めるかは Phase 3 以降で検討（初期ルールには含めない）
