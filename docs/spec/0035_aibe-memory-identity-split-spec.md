...
1. request に明示された `memory_space_id`
2. `AIBE_CONTEXT_ID` 環境変数
3. `cwd` から導出した project-backed `memory_space_id`
4. `legacy_session_<session_id>`

`ai` は可能な限り 1 〜 3 を埋めて送る。  
`aibe` は旧クライアント互換のため 4 までフォールバックできるようにする。
...
resolution は次の 2 段に分ける。

1. `cwd` から `project_key` を求める
2. `project_key` または env / explicit id から `memory_space_id` を求める
...
| コマンド | 挙動 |
|----------|------|
| `ai context current` | 現在の解決結果を表示する。優先順位は `AIBE_CONTEXT_ID` > local config > cwd project-backed id > legacy session |
...
`ai` は memory 系 RPC を送るとき、解決済みの `memory_space_id` を `MemoryContext` に載せる。
...
- `AIBE_CONTEXT_ID` が project-backed id より優先されること
- `legacy_session_<session_id>` fallback が働くこと
