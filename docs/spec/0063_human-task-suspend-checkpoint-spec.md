...
- `lock` は0600/current UID/symlink拒否で開き、`flock(LOCK_EX)` によりcreate/status/cancelのroot操作を直列化する
...
createはlockをactive確認前からHuman Shell終了後の最終save/removeまで保持する。statusとcancelも同じroot lock下でloadし、cancelは同じlock下でremoveする。これにより複数`ai` process間でもsingle-active-task invariantと一貫読取を守る。flockはoperation-scoped exclusionであり、process消失後の所有権を表すlease/heartbeatではない。
...
CLIはaibe socketへ接続せずlocal storeだけを参照する。statusはroot lock下で一貫して読み、Suspended taskがある場合はtask ID、`suspended`、objective、suspended time、任意reason、current cwd、`ai human-task cancel --yes` を表示してexit 0とする。taskがない場合は厳密に `No suspended Human Task.` を表示してexit 0とする。

cancelはroot lock下でSuspendedを再確認してremoveする。`--yes`なしはTTY stdinで確認し、非TTY、拒否、入力失敗はnon-zeroかつfile非変更とする。taskなしはstatusと同じ成功文でexit 0とする。Running、破損、未知version、権限不正は「taskなし」へ丸めず、削除しない。cancelはagent continuationやCancelled tool resultをaibeへ送らないlocal復旧に限定する。
