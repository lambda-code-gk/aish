...
project scope は **同一 `AI_SESSION_ID` 内で `project_key` により論理分離される**。物理保存は session 配下の JSONL のままで、別 `AI_SESSION_ID` へ goal / idea を引き継がない。これは MVP の非目標である。
...
`id` / `session_id` / `version` は永続化と競合制御のための必須メタデータであり、`MemoryEntryDto` では省略しない。
...
| `idea` | `project` | `on_demand` | `active` |

`resolve_for_prompt` と通常注入では `active` の entry のみを対象にする。`inactive` と `archived` は保持用であり、自動注入しない。

## 設計概要

### 保存場所

memory の正本は session 配下の JSONL event store とする。
...
- `now` は session-scoped として通常の `agent_turn` に自動注入する
...
シリアライズ上のタグは `op` とし、variant は `Add` / `ClearActive` / `Archive` を使う。
...
```text
MemoryOperationDto = Add | ClearActive | Archive
...
- `idea add` は project-scoped entry を追記し、`kind=idea` / `scope=project` / `inject=on_demand` / `status=active` を既定とする（CLI 上の open 相当）。`make_active=false` とし、既存の open を変更しない
...
- `goal clear` / `now clear` / `idea clear` は `op: "clear_active"` で wire できる
- `ai mem clear` は `op: "clear_active"` で wire でき、必要なら `kind` / `scope` を明示して同じ wire に寄せられる
...
`entries` は操作で影響を受けた entry 群を返す。`clear_active` では複数の inactive 化対象をそのまま表現でき、`archive` では単一 entry の状態遷移を返す。
...
- append / upsert / clear_active / archive
- replay / materialize
- project_key による絞り込み
- session scoped store への永続化
...
- ディレクトリは 0700
- `events.jsonl` は 0600
- session 単位の lock を使い、会話 store と同じく競合を書き潰さない
...
`goal` は現在の `cwd` から導出した project_key に対して、`now` は session に対して、`idea` は現在の `cwd` から導出した project_key に対して解決する。`now` は session-scoped なので project_key の違いで切り替わらない。`active` 以外の entry は通常の prompt 注入対象にしない。
...
- 途中で budget を超える場合、UTF-8 boundary を壊さずに切り詰める
...
| `ai mem clear` | `MemoryApply` を送る。`kind` / `scope` / `cwd` に一致する active entries を inactive 化する。送信する operation は `ClearActive` |

`ai goal clear` / `ai now clear` / `ai idea clear` は、それぞれ固定 `kind` と `scope` を持つ `ClearActive` の薄いラッパーである。
...
AI_SESSION_ID の供給は既存の smart entry と同じ経路を使う。
...
session 配下の memory store は conversation store と同じく、読取 0600 / ディレクトリ 0700 を前提にする。
...
- 4KB budget を超える場合に UTF-8 boundary を壊さず切り詰めること
...
- `ClearActive` と `Archive` の serde round-trip
...
- session 単位の lock が同時実行時に競合を書き潰さないこと
- 同時 append が失われないこと
...
1. `aibe` に session-scoped memory store が追加される
2. `~/.local/share/aibe/conversations/<AI_SESSION_ID>/memory/events.jsonl` に保存される
