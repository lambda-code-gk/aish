...
- `LocalRouteKind::VcsInspect` の wire 値は `vcs_inspect`
- `VcsInspect` は `context_needs`（`VcsStatus` / `VcsDiff`）または route kind 既定に対応する git tool capability が CLI allowlist に無い場合、`fallback_required=true` とする
...
- local route の初期対象は `simple_chat`、`shell_help`、`vcs_inspect`、`output_style_request`、`code_review_context_selection` に限定する。`memory_query` は local fast path から除外し、0041 / 0042 の `route_turn` + `feature_executor` 正本経路に残す
...
- high confidence かつ safe な `simple_chat`、`shell_help`、`vcs_inspect`、`output_style_request`、`code_review_context_selection` は local route に入る
...
- 高信頼かつ safe な local_route 対象のみを短絡候補にする（初期 allowlist は `simple_chat` / `shell_help` / `vcs_inspect` / `output_style_request` / `code_review_context_selection`）
...
- `simple_chat`、`shell_help`、`vcs_inspect`、`output_style_request`、`code_review_context_selection` が local route に入ること
