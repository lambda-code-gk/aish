...
- 親端末の raw 化対象と stdin relay 元を同じ fd に揃える限定修正を超える PTY relay / handoff lifecycle 変更
...
| `handoff_recall_preserves_line_editor_navigation` | CI の outer PTY 上で `CollaborativeMockServer` と実 `ai → aish human-shell → PtyShell::run_shell` 経路を bash / zsh で実行し、通常 CSI と handoff 候補の `Alt+.` 後の CSI が line editor に解釈される。rcfile 単体の両 shortcut・上下 history・termios・候補なし binding 所有は補助回帰で維持する |
...
| `aish_shell_alt_period_preserves_csi_navigation` | 実 `aish shell`（replay DEBUG 有効）で `Alt+.` 直後の CSI 左右が caret 挿入にならず、line editor で既存入力を編集できる（Issue #11） |
...
| `handoff_recall_preserves_line_editor_navigation` | 0055 `CollaborativeMockServer` を使う outer-PTY 製品経路 E2E | 実 `ai` / `aish` と bash / zsh の両方を実行する。shell 不在は skip しない |
| `recall_hooks_avoid_unobserved_terminal_reset` | 生成 hook の契約試験 | 文字列検査は reset 禁止の補助ゲートに限定し、navigation 達成の代用にしない |
| `aish_shell_alt_period_preserves_csi_navigation` | 実 `aish shell` + outer PTY | replay DEBUG が有効な製品経路で Alt+. 直後の CSI を検証する（Issue #11 再現） |
...
| 3 | BLOCKER_ORIGINAL_AC / REGRESSION | 0055 AC を outer-PTY の実 `ai → aish → PtyShell` 経路へ移し、親 stdin が redirect 済みでも controlling TTY の同一 fd を raw 化・relay・winsize 同期に使う限定修正を許可 | rcfile 直起動試験が製品の親 raw TTY と stdin relay を迂回し、実利用の CSI 回帰を検出できなかったため |
| 4 | REGRESSION / SAFETY_WITHIN_FAULT_MODEL | `aish shell` の replay `DEBUG` trap が `bind -x` 中に start span を出すと readline が壊れ CSI が caret 挿入になる経路を AC 化し、`FUNCNAME` で recall/handoff widget を抑止 | Issue #11 は候補有無に依らず Alt+. 直後に再現し、stdin 分離だけでは不足だったため |
...
1. **確認済み（Issue #11 / `aish shell`）**: `aish` の replay `DEBUG` trap（`_aish_replay_debug`）が bash `bind -x`（`Alt+.`）中の simple command ごとに走り、`start` span を control fifo へ書くと readline が壊れ、以降の CSI が `^[[D` として挿入される。候補の有無に依らない。抑止は `FUNCNAME` で recall/handoff widget を除外する（flag だけでは DEBUG が代入前に走るため不十分）。
2. **部分確認（第一仮説）**: line editor widget から起動した `ai recall` subprocess が widget の stdin を継承し得る。stdin を `/dev/null` へ分離する。
3. **部分確認（第二仮説）**: zsh の `zle reset-prompt` は line editor を乱し得るため `zle -R` へ寄せる。
4. **推測（第三仮説）**: `ai complete` と Human Shell wrapper の同一キー binding が install 順で上書きされ、想定と異なる widget が動作している。PTY 内で `bind -S` / `bindkey` と実行 widget を観測して切り分ける。
5. **推測（切り分け対象）**: ESC sequence の部分消費または termios 残留。観測根拠なしの `stty sane` は採用しない。
...
5. `aish shell` の replay `DEBUG` は recall/handoff `bind -x` 中に start span を出さない（`FUNCNAME`）。
6. PTY 試験を主回帰とし、生成 script の文字列検査だけで AC 達成とはしない。
