...
> **種別**: 設計書（`docs/spec/`）  
> **状態**: 設計確定  
> **関連**: [`docs/feature-development-policy.md`](../feature-development-policy.md)、[`0045_pack-composition-spec.md`](0045_pack-composition-spec.md)、[`0055_minimal-human-handoff-spec.md`](0055_minimal-human-handoff-spec.md)、[`0057_pty-process-cleanup-hardening-spec.md`](0057_pty-process-cleanup-hardening-spec.md)、[`0059_collab-outcome-status-spec.md`](0059_collab-outcome-status-spec.md)
...
本仕様は 0059 の対話収集 UX を置き換え、Human Shell 終了後の `done` / `blocked` / `cancelled` 選択を除去する。summary 入力も設けない。Ctrl+D または `exit` で Human Shell が終了した後は、追加入力なしで既存の同期 handoff 経路から親エージェントへただちに制御を返す。

新規 protocol field は追加しない。0059 が追加した既存 `HumanHandoffResult.collab_outcome` は必須から optional（Rust では `Option`、`None` は `skip_serializing_if` で省略）へ緩和し、成功した handoff では付与しない。exit code、return marker、表示内容などから推定した status も埋め込まない。これは新しい schema の追加ではなく、0059 の必須契約の撤回である。0059 の対話収集と必須 DTO を前提とする AC は、0060 実装時に pending 化、削除、または本仕様の AC へ差し替える。
...
- 新 protocol field、新 shell_exec request/response、新環境変数、新設定、外部依存（既存 `collab_outcome` の optional 化は除く）
...
| 他機能統合 | 1（0055/0057 の既存 handoff 経路上で 0059 outcome 手入力を除去） |
...
- 分割判断: briefing 表示と 0059 の終了後手入力除去だけを同じ即時 return UX として扱い、task 管理・結果収集・非同期協調は Deferred specs へ送る
- 承認例外: なし
...
| protocol / schema / env | 新規 +0（既存 `collab_outcome` の required → optional 緩和のみ） |
| 新規基盤機構 / 外部依存 | +0 |
...
| `human_task_briefing_returns_with_ctrl_d_or_exit` | briefing が Ctrl+D と `exit` の双方を案内し、どちらでも追加入力なしにただちに親 agent へ制御を返す |
...
| `human_task_briefing_has_no_outcome_selection` | Human Shell 終了後に 0059 の `done` / `blocked` / `cancelled` 選択を表示せず要求しない |
| `human_task_briefing_has_no_summary_input` | Human Shell 終了後を含む handoff 全体で summary または理由の手入力を要求しない |
| `human_task_briefing_adds_no_protocol_schema` | 新しい protocol field、request、response、tool を追加せず、既存 `HumanHandoffResult.collab_outcome` を optional 化して成功 handoff では省略し、推定 status を埋め込まない |
...
| 2 | CONTRACT | 0059 の対話収集 UX を撤回し、既存 `collab_outcome` を optional 化。trim 空値、両複数行 field の行単位 indent / escape、製品 AC と完了ゲートの分離を明確化 | 手入力を完全に除去しつつ新規 protocol field や推定 status を導入せず、安全な表示契約と Scope Lock を一致させるため |
