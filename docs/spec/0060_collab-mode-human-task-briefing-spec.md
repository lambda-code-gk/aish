...
| `human_task_briefing_adds_no_protocol_schema` | `collab_outcome` / `CollabOutcome` / `CollabOutcomeStatus` を protocol source へ再導入しない（静的検査。serde skip だけでは不十分） |
...
| 5 | HARDENING | `adds_no_protocol_schema` を source 静的検査へ変更。zsh handoff で `skip_global_compinit` をユーザー `.zshrc` 前に unset | serde skip では再発を検出できず、また CI 用フラグがユーザー補完初期化へ漏れるため |
