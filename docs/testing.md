...
`verify.sh` はワークスペース全体の fmt / clippy / test / 静的検査を **直列**で回すため、数分かかるのが正常である。体感が悪くなる典型原因は次のとおり。

| 原因 | 対策 |
|------|------|
| `\| tail` / `\| head` で包む | **使わない**。各段階は `==> …` と `done (Ns): …` で即時表示される |
| 実装中に毎回 `verify.sh` | `verify-targeted.sh --package <crate>` を使う |
| テストだけ絞りたい | `VERIFY_PACKAGES=ai ./scripts/verify.sh`（fmt/clippy/静的検査はフル） |
| 別ターミナルで経過を見たい | `VERIFY_PROGRESS=1 ./scripts/verify.sh` → `tail -f .verify-progress` |

`verify.sh` 実行前に `cargo fmt` を重ねて走らせない（スクリプト内で `fmt --check` 済み）。
...
### 0055 Collaborative durable workflow

- `ai/tests/0055_collaborative_workflow.rs`: aggregate atomic round-trip、reducer invariant、stale pending effect reconciliation、token plaintext 非永続化。
- `scripts/check-collaborative-workflow.sh`: application の split save 禁止、workflow store / reconciler 配置、token plaintext 境界を検査する。
- `ai/tests/0055_collaborative_vertical_slice.rs`: 実 shell を含む縦切り。full PTY 2件は `scripts/run-collaborative-nightly.sh` で実行する。
