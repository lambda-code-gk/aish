...
- `ai/tests/0064_human_task_resume_red.rs`: Suspended→resume→再suspendのvertical E2E、cwd/briefing復元、segment追記、root flock保持、複数回suspendのObservation順序、missing/mismatched ID、非Suspended拒否、cwd不在、Done時のResultPending永続化、再suspend最終save失敗時のSuspended復元、Done最終save失敗時のRunning維持、起動後Cancelled/Interrupted/FailedのRunning維持、pre-launch失敗のSuspended復元、ParentTermiosGuard配線、0063単一segment/Done削除回帰。
