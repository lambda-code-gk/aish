...
`--test`はCargoのintegration test target名で、複数回指定できる。パッケージ検証では`cargo fmt --all -- --check`、対象パッケージの`cargo check`、対象テストを順に実行する。`aibe-client`は先に`aibe`をbuildし、テストへ`--test-threads=1`を付ける。すべてのcargo testは`-j 1`で実行する。
...
| 並列に戻す | RAM に余裕があるマシンのみ `VERIFY_PARALLEL=1 ./scripts/verify.sh` |
...
`memory` feature off 時、contextual memory の integration test は crate 先頭の `#![cfg(feature = "memory")]` で除外される。unit test は `plugin_memory` 内の `#[cfg(test)]` が同 feature に従う。
...
Phase 4 の AC は同ファイル群に `#[ignore]` で先行配置し、該当 Phase 完了時だけ ignore と `pending` を同時に解除する。feature-off は `cargo test -p ai -j 1 --no-default-features --lib work_cli_stub_rejects_when_memory_feature_is_disabled` でも確認する。手動手順は [manual/ai-work.md](manual/ai-work.md)。
