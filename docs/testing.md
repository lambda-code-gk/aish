...
設計: [spec/0044_smart-preprocessor-spec.md](spec/0044_smart-preprocessor-spec.md)。
...
| shadow / assist mode 導通 | `ai/tests/smart_preprocessor_ask_e2e.rs`（integration、TTY + mock aibe） |
| non-TTY は preprocessor / route_turn を飛ばす | `ai/tests/phase_a_cli.rs` |
| Observation Log の redaction / telemetry 項目 | `ai/src/application/` または `ai/src/domain/`（unit） |
| `--preset` / `--tools` / `--yes-exec` 明示値の優先 | 既存 smart entry / smart feature テスト |
