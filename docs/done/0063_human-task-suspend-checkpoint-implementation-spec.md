...
- Scope revision: **2**（implementation scope lock）
- Status: `locked`
...
**Vertical Slice Gate**: Phase 1成功前にresume、複数segment、continuation、crash recovery、schema migration、file lock/lease、汎用永続framework、性能最適化へ進まない。
...
| `human_task_status_reports_suspended_checkpoint` | 1 | `ai/tests/0063_human_task_suspend_checkpoint_red.rs` | socket不在、許可field、local時刻、任意reason、resume案内、exit 0 |
...
| 候補ファイル | 作業 |
|--------------|------|
| `ai/src/ports/outbound/human_task_store.rs`（新規）、`ai/src/ports/outbound/mod.rs` | `HumanTaskStore`のload/save/removeをdomain型と安定errorで定義する。applicationからpath/JSON/fs APIを隠す |
...
portのerrorへcheckpoint本文、objective、元ユーザー要求、reason、秘密候補値を含めない。removeはterminal Done/Blocked/Cancelled後の0063 checkpoint cleanupにだけ使い、invalid checkpointを消す回復APIにはしない。

### 3.3 Adapters

| 候補ファイル | 作業 |
|--------------|------|
| `ai/src/adapters/outbound/human_task_file_store.rs`（新規）、`ai/src/adapters/outbound/mod.rs` | `<history_dir>/human-tasks/<id>/checkpoint.json`を0700/0600、current UID、component symlink拒否、`O_NOFOLLOW`、1 MiB上限、same-dir temp→file fsync→rename→parent fsyncで実装する |
...
secure storeの低水準処理は既存secure filesystem primitiveを再利用してよいが、aibe file-change journal、二つ目の永続正本、lock/lease/reconcilerを導入しない。atomicity test用のfault injection seamはadapter内部のwrite operations traitまたは既存primitive境界に限定し、production pathと別実装を作らない。
...
| `ai/src/application/human_task_status.rs`（新規候補） | store read-only load、Suspended invariant、表示model生成を行う。taskなしだけをsuccess emptyへ写像する |

Running保存失敗はshell開始前に `human_task_checkpoint_unavailable`。既存非終端は `human_task_already_active`。Done/Blocked/Cancelledのcleanup失敗を成功に丸めず、既存有効checkpointを破壊しない。単一process内のcoordinator呼出しはcomposition rootで直列化するが、file lockやprocess ownershipは作らない。
...
固定応答は設計書の本文と改行を正本とし、LLM、objective、reason、Observationから生成しない。statusはtask ID/state/objective/time/reason/cwd/resume案内以外を表示しない。
...
8. **Status**: read-only application serviceとCLI parse/dispatchを接続し、socket不使用、empty/invalid区別を実装する。
...
12. **完了処理**: targeted検証後に`./scripts/verify.sh`。全16 ACとverify成功後だけstatus=`done`、本書を`docs/done/`へ移動しindexを実装済みにする。
...
- `docs/manual/0063_human-task-suspend-checkpoint.md`（新規）: 明示Human Task開始、reasonあり/なしsuspend、別command status、通常Done、validation失敗、未実装resume案内を記載する。
- `docs/manual/README.md`: 0063手動検証へのリンクを追加する。
- `docs/0000_spec-index.md`: 実装中はtasks rowを維持し、全AC完了後だけ実装済みへ変更する。
- CLI help: `human-task status`だけを公開し、resume/cancelが実行可能だと誤記しない。
...
resume / cancel / continuation / crash recoveryは設計書のNon-goals / Deferredどおり実装しない。仮serializer、in-memory production store、常に成功するlauncher、固定Suspended result、source文字列検査で本機能を完了にしない。
