...
| `ai/tests/0055_collaborative_handoff_vertical_e2e.rs` | `ai` acceptance / outer-PTY Human Shell E2E | `CollaborativeMockServer` と実 `ai → aish → PtyShell` 経路で bash / zsh の通常 CSI と handoff recall 後 CSI を検証する |
| `aish/tests/0067_recall_keybinding_tty_restore.rs`、`ai/tests/0057_pty_process_cleanup_hardening.rs` | rcfile 補助回帰 / 既存 PTY 試験の参照元 | 両 shortcut・history・termios・binding 所有の補助回帰と、`openpty`、controlling TTY、bounded wait、cleanup の実装パターンを維持する |
...
- 親端末の raw 化対象と stdin relay 元を同じ fd に揃える限定修正を超える PTY relay / handoff lifecycle または PTY 基盤自体の変更
