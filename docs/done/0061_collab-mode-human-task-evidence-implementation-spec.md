...
`aish` の本番コード、ログevent schema、hook、環境変数は変更しない。test fixtureの利用を除き `aish` に0061ロジックを入れない。
...
- `aish` 本番コードまたは0049 span semanticsの変更
...
- [ ] `aish` 本番コード、port signature、`ai/src/main.rs` の責務を拡張していない
