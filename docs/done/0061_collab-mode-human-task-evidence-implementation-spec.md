...
3. control FIFO emit を通常 `wait` + 別 watchdog（約 0.5s で writer kill）にする（busy spin / 新 FIFO は置かない）
4. `handle_control_line` で後続 start/end を `VecDeque` 相当へ保持する（上書き禁止・高速連続入力の span 欠落防止）
...
- `aish` 本番コードへの**新規** hook / 環境変数 / ログ event、または既存 0049 replay hook の exit_code/FIFO/lifecycle queue 修正を超える span semantics 再設計
