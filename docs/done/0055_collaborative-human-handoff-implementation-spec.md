...
1. Phase 1–5 の `spec-acceptance.toml` がすべて `pending = false`
...
正本: 設計書 spec §32 の表。`scripts/spec-acceptance.toml` の `spec = "0055"` は **83 件**（Phase 1: 9, 2: 20, 3: 26, 4: 16, 5: 12）。各 Phase 指示書 §2 と 1:1。
