...
実測結果は adapter の unit test または `../manual/` に追記する。0001 プロバイダテスト拡張は別途可。
...
|-------------|-------------------|---------------|
| `role: system`（wire `MessageRole::System`） | **含める**（無加工） | **含める**（ループ会話内の出現順のまま、`initial_user_request` より前） |
...
**注**: user メッセージの **`[system]` content 前置**（注入用）と **`role: system`** は別物。前者は `initial_user_request` の除外対象、後者は上表のとおり終端入力に **保持** する。
...
```rust
// summary.rs — SummaryPrompt（0008: system 保持）
...
`initial_user_request` の除外プレフィックス（0003 維持）: `[shell log tail]` / `[system]` **（user content 前置）** / `## Tool execution results`。`role: system` メッセージは `system_messages()` で別途保持する。
...
- **ログ**: application が `TerminationResult.strategy` を `tracing` で 1 行出力（例: `termination strategy=conversation_replay`）。秘密・全文 conversation は出さない（`../security.md` 準拠）。
...
7. 実装と同 PR で **`../architecture.md`** に終端戦略・capability・設定キーを追記する。
8. **`../manual/ai-ask-tools.md`** に max-round 到達時の確認手順（Replay 設定は任意・実 LLM）を追記する。
...
| 単体 | `summary.rs` | `summary_prompt_preserves_system_messages` | ループ会話先頭の `role: system` を SummaryPrompt 終端入力に **含める** |
...
| 手動 | `../manual/ai-ask-tools.md` | — | max-round 到達例（実 LLM・任意） |
...
- 手動検証（実 LLM・各プロバイダ）は `../manual/ai-ask-tools.md` 手順追加後に実施推奨
