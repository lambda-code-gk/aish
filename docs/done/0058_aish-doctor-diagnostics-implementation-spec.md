...
5. `output_filter_configuration`: `resolve_filter_metadata(AI_FILTER, None, cfg.ask_filter)` の `enabled/source/masked` を使う。未設定は利用可能な通常状態として OK、設定済みかつ source が解決し masked なら OK、解決不能は FAIL。filter command 本文は絶対に含めない。
