...
| **安全性** | API キーは aibe 設定に限定し、ログとリポジトリに残さない。`docs/security.md` の `AIza` マスク方針に従う。 |
...
- `name` が `ToolName` にパースできない場合 → `LlmError::UnknownTool`（`openai_compatible` と同じ）

## ツール名エラーと 0001 の区別

| 状況 | 挙動 |
|------|------|
| モデルが **組み込みに存在しない** ツール名を返す（`ToolName` パース不可） | `LlmError::UnknownTool` → turn `error`（`tool_not_allowed`）。`openai_compatible` と同型。 |
...
10. 組み込みに存在しないツール名の `functionCall` → `LlmError::UnknownTool` → turn `tool_not_allowed`（`openai_compatible` テストと同型）。
...
13. `docs/security.md` の `AIza` マスク方針と矛盾しない。
...
| `agent_turn_unknown_tool_from_llm_returns_tool_not_allowed` | `openai_compatible_llm.rs` と同型 |

### 手動

- [docs/manual/gemini-provider.md](manual/gemini-provider.md)
...
| `docs/aibe.config.example.toml` | gemini 設定例（コメント） |
| `docs/manual/gemini-provider.md` | 手動検証（実装後に cwd 注記済み） |
| `AGENTS.md` | 0010 行追加 |
...
| ドキュメント | 関係 |
|-------------|------|
| **0001** | エージェントループは不変。allowlist 外の既知ツールは tool result で継続。組み込み未知名は adapter が `UnknownTool` で turn 終了（openai 現行と同じ）。 |
...
- `docs/architecture.md` — Gemini provider、`generateContent`、`provider_extras`
- `docs/security.md` — 変更なし想定（`AIza` マスク既存）
- `docs/aibe.config.example.toml` — gemini 例
- `docs/manual/README.md` — 索引（済なら維持）
- `AGENTS.md` — 0010 状態を実装済みに更新
- `docs/0000_spec-index.md` — 0010 を実装済みに更新
- `docs/0006_max-tool-rounds-terminator-spec.md` — Gemini 行の「推測」は manual 実測後に更新可
