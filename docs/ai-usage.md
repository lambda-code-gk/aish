...
- 省略した場合:
  - `aish sysq` で管理されているシステムプロンプト（`system.d` 配下の有効なエントリ）が自動的に解決され、結合された文字列が system instruction として使われます。
  - sysq / system.d の詳細は `system-prompts.md` を参照してください。
