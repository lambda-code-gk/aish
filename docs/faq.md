...
### システムプロンプト（sysq）が効いているか分からない

- `aish sysq list` で、各スコープで有効なシステムプロンプトを確認できます。
- `ai -S "..."` を指定している場合は、sysq は使われず、指定した文字列がそのまま system instruction になります。sysq を試すときは `-S` を付けずに実行してください。
- 詳細は [system-prompts.md](system-prompts.md) を参照してください。
