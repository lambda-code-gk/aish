...
3. `human-task suspend '承認を確認してから続ける'`を実行する。shellが終了し、親turnが`Human Task suspended.`とtask ID、`ai human-task resume`と`ai human-task cancel --yes`案内を表示して正常終了することを確認する。
4. aibe/aiのturn終了後に別の`ai human-task status`を実行し、task ID、state、objective、local time、reason、cwd、Resume/Cancel案内が表示されることを確認する。
...
`ai human-task resume`は0064で実装済み。continuation、crash recovery、lease/heartbeat、schema migrationは未実装である。cancelはlocal checkpoint削除だけでagent continuationを行わない。
