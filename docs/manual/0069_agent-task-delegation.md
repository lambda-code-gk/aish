...
- [ ] Resultのstatus/stdout/stderr/exit/timeout/truncationとchanged path Evidenceがboundedに表示される
- [ ] Resultと全Evidenceが`verified=false`で、Workerの完了自己申告だけで親が完了断定しない
- [ ] timeout時にWorkerと子孫processが残らず、成功扱いにならない
- [ ] approval/audit/errorにcredential値、raw env、file contentが表示されない
