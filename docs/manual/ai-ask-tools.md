...
- **stderr** に `ai: shell_exec approval required:`、`tier:`、`command` / `args`、選択肢 `choices: [y]es / [n]o / [a]lways-this-session / [c]ommand-only` が表示される
- `Execute? [y/n/a/c]` プロンプトが **stdout ではなく stderr**
- `n` または空 Enter で拒否され、turn は `tool_calls` に `approval_source`（例: `shell_exec_approval=ask;ui=n`）付きで継続可能
- `read_only` tier（例: `git status`）は session 許可後、追加 prompt なしで通る場合がある（0036）
- `destructive` tier は毎回 prompt される
- `command` / `args` は制御文字を **escape した 1 行**（`\x1b` 等）で表示され、raw ANSI による見た目偽装ができないこと
- `y / n / a / c` の 4 選択で、`a` は exact invocation、`c` は command 名を session 内に記憶する（destructive は毎回 prompt のまま）。
- `read_only` は session 許可後に自動承認、`mutating` は記憶または pattern があるときのみ自動承認。
- 自動承認時は **stderr** に `ai: shell_exec auto-approved (...): <command>` が **既定で** 出る（手動承認時は上記 prompt で command が見える）。`--silent-exec` または `--quiet` で抑止。
- `[tools.shell_exec.auto_approve_patterns]` は session 許可後のみ評価される。

#### B3c. 非対話 stdin（fail-closed）
...
- **stderr**: `ai: shell_exec denied (non-interactive stdin)`（`y` を流しても実行されない）
- turn は拒否記録付きで継続可能（実 LLM + allowlist が必要な場合は B3b と同様）

（mock LLM だけでは B3b を再現しない。実 LLM + `@exec` + allowlist 設定が必要。）

#### B3d. session 許可後の tier / pattern

`a` または `c` を 1 回通した同一 session では、`read_only` tier は session 許可後に自動承認され、`[tools.shell_exec.auto_approve_patterns]` がある場合は `read_only` / `mutating` tier で pattern による自動承認が働く。`destructive` tier は毎回 prompt のまま。
