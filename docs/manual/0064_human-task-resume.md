...
6. 再度`ai human-task resume`し、Ctrl+Dまたは`exit`で終了する。`result pending`として保存され、statusにResume案内が出ないこと、再resumeが拒否されることを確認する。
7. cwdを削除したfixture相当でresumeするとshellが起動せずSuspendedが維持されることを確認する（任意）。
8. `ai human-task cancel --yes`でResultPendingを削除できることを確認する。

## 期待結果

- resumeはaibe未起動でも動作する。
- 再suspendでsegmentが積み上がる。
- Done後はResultPendingとしてEvidenceを保持し、agent continuationはまだ案内・実行されない。

## 未実装

- ResultPendingからのCollaborative Mode continuation（0063-D）
