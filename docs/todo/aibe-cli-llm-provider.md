# aibe — CLI ベース LLM プロバイダ（旧案・非採用）検討メモ

> **状態**: 旧案（非採用）— 0024 / 0025 の first-class 統合案は採用しない。代替設計は [0026_external-commands-spec.md](../spec/0026_external-commands-spec.md)  
...
> **更新**: 0026 で、CLI の呼び出しは `shell_exec` の外部コマンドとして扱う方針に切り替えた。したがって本メモの `LlmProvider` 化・`invoke_*` 化は歴史的背景にとどめる。
...
このメモの当初案は採用しない。AISH で CLI coding agent を使う場合は、`shell_exec` の外部コマンドとして明示登録し、AISH の新しい LLM プロバイダにはしない。
...
- [ ] 実装時: 設計は [0026](../spec/0026_external-commands-spec.md)、実装指示は `docs/tasks/` に作成し、完了後 `docs/done/` へ移す
