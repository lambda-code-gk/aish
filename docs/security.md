...
- cancelはroot lock取得後のSuspended、orphaned Running、またはdelete失敗後のFinishedだけを削除する。`--yes`なしの非TTY・拒否・入力失敗、およびinvalid checkpointでは削除しない。有効task ID directoryのcheckpoint欠落やtemp残骸はInvalidとして保持し、aibeへagent continuationやCancelled tool resultを送らない。
...
- continuation中は同じroot flockを保持し、別ai processの二重開始を防ぐ。aibeは`continuation_turn=true`のactive IDと正常完了IDをprocess memoryだけで拒否する。永続dedup正本、aibe再起動跨ぎexactly-once、Continuing crash recoveryは提供しない。
