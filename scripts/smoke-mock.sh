...
echo "smoke-mock: ai doctor human and json..."
timeout 30s "$AI_BIN" doctor --quiet --socket "$AIBE_SOCKET_PATH" >"$STDOUT_FILE" 2>"$STDERR_FILE"
grep -q 'socket_reachable' "$STDOUT_FILE" || fail "ai doctor human must include socket_reachable"
[[ ! -s "$STDERR_FILE" ]] || fail "ai doctor --quiet must suppress stderr diagnostics"
timeout 30s "$AI_BIN" doctor --quiet --socket "$AIBE_SOCKET_PATH" --format json >"$STDOUT_FILE" 2>"$STDERR_FILE"
grep -q '"command":"doctor"' "$STDOUT_FILE" || fail "ai doctor json must use DoctorReport"
grep -q '"id":"protocol_compatibility"' "$STDOUT_FILE" || fail "ai doctor json must include protocol check"
