...
*   **Memory System**: A persistent "Search then Fetch" memory architecture that allows the agent to store and retrieve past successes, code patterns, and project-specific knowledge.
*   **Security Scanning**: Integrated high-performance `leakscan` (Rust) to prevent accidental transmission of API keys, passwords, and other sensitive data.
